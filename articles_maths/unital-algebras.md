...
Throughout, $R$ is a commutative ring with identity $1 \neq 0$ and $A$ is an $R$-algebra in the broad sense, unless a sentence names a stronger hypothesis. The article is algebraic throughout and uses no distance, no norm and no limit; the one notion of the corpus that is defined by convergence, the approximate identity, is stated in §*Approximate Identities* for the sake of the examples, and it belongs to *The Convolution Algebra $L^1(G)$*.

The aim of the article is the equivalence of §*Unital Algebras as Rings over the Centre*: a unital $R$-algebra is a ring with identity together with a unital ring homomorphism $R \to Z(A)$, and conversely. From it, the article treats the trivial cases and the standing hypothesis $1 \neq 0$, the unit group and its specialisations, the unitisation $A^+$ by which a unit is adjoined to any algebra, the homomorphisms that preserve the identity and those that do not, and the idempotents, whose central ones decompose a unital algebra as a product. It closes with the non-unital algebras of the corpus and with the approximate identities that its topological ones carry instead of a unit.
...
1. $\eta$ is a unital ring homomorphism, $\eta(1_R) = 1_A$ and $\eta(r + s) = \eta(r) + \eta(s)$, $\eta(rs) = \eta(r)\eta(s)$;
2. the image $R \cdot 1_A = \eta(R)$ is contained in the centre $Z(A)$;
3. $R \cdot 1_A$ is the smallest unital $R$-subalgebra of $A$, and $\eta$ is the unique unital $R$-algebra homomorphism $R \to A$.
...
1. an associative unital $R$-algebra structure on a set $A$, with product $A \times A \to A$;
2. a ring structure with identity on $A$, associative, together with a unital ring homomorphism $\eta : R \to Z(A)$ into its centre.
...
**Remark.** The theorem explains the convention, stated in *introduction-and-mathematical-conventions*, that a module is written with its scalars on the left and that the side begins to matter when the ring is non-commutative. For a unital algebra over a commutative $R$ the scalars lie in the centre, so the left and right actions agree, and no side is preferred; the centrality of $\eta$ is what makes the notation unambiguous.
...
1. In the matrix algebra $M_n(R)$ the units are the matrices of invertible determinant,
...
5. In the algebra $C(X)$ of continuous complex-valued functions on a compact Hausdorff space, with the pointwise operations, the units are the functions that vanish nowhere: a unit cannot vanish at a point, and conversely a function with no zero has the pointwise reciprocal as its inverse, continuous because $|f|$ attains a positive minimum on the compact space $X$. The algebra is the commutative unital case of the identification of *Operator Algebras*, §*The Gelfand–Naimark Theorems*, where a commutative $\mathrm{C}^*$-algebra is $C_0(X)$ and is $C(X)$ precisely when $X$ is compact; the same statement for the smooth algebra $C^\infty(S^1)$ is used in *Cyclic Cohomology*, §*The Pairing with K-Theory*, where an invertible element of $C^\infty(S^1)$ is a nowhere-vanishing smooth function.
...
1. the element $(0, 1)$ is an identity;
2. the inclusion $a \mapsto (a, 0)$ is an injective algebra homomorphism whose image $A \oplus 0$ is a two-sided ideal;
3. the quotient map $A^+ \to R$, $(a, r) \mapsto r$, is a unital algebra homomorphism with kernel $A \oplus 0$, so that $A^+/A \cong R$;
4. $A^+$ is associative if and only if $A$ is.
...
1. $\varphi(1_A)$ is an idempotent of $B$, and $\varphi$ is unital if and only if that idempotent is $1_B$.
...
- **A proper ideal of a unital algebra.** A proper two-sided ideal $I$ of a unital algebra $A$ contains no unit of $A$, so $1_A \notin I$ (*Ideals and Quotients of Algebras*, §*Ideals*); an ideal of this kind is non-unital as a subalgebra of $A$, but it may carry an identity of its own, an idempotent of $A$ different from $1_A$, and then it is a unital algebra. The two cases occur: $(x) \subset R[x]$ is non-unital, while the augmentation ideal $I(G) \subset k[G]$ of a group algebra is spanned in the case $G = \mathbb{Z}/2$ over a field of characteristic not $2$ by $1 - g$ with $(1-g)^2 = 2(1-g)$, so it carries the identity $(1-g)/2$ and is unital (*Group Algebras*, §*The Augmentation Ideal*).

- **The convolution algebra $L^1(G)$.** For a locally compact group $G$ the convolution algebra of *The Convolution Algebra $L^1(G)$*, §*Associativity, Involution and the Banach Algebra Structure*, is associative and has no identity when $G$ is not discrete, while for discrete $G$ it is unital with identity the point mass $\delta_e$; the algebra is therefore unital exactly when $G$ is discrete. The statement is quoted from that article, where the convolution, the involution and the norm are set up; the algebra is not an object of Part I.

- **The algebras of compact operators.** Let $H$ be an infinite-dimensional Hilbert space. The algebra $B(H)$ of bounded operators is unital, and the algebra $K(H)$ of compact operators is a proper two-sided ideal of it; *Operator Algebras* states that $K(H)$ is a $\mathrm{C}^*$-subalgebra without identity unless $H$ is finite-dimensional, and that $K(H)$ is simple. Hence a simple algebra need not be unital, and the unitisation $K(H)^+$ is the unital $\mathrm{C}^*$-algebra in which $K(H)$ sits as an ideal with $K(H)^+/K(H) \cong \mathbb{C}$. The compact operators themselves, their norm and their closedness are set up in *Banach and Hilbert Spaces*, in Part III.

- **The algebra $C_0(X)$.** For a locally compact Hausdorff space $X$ the algebra $C_0(X)$ of continuous complex functions vanishing at infinity is commutative and unital exactly when $X$ is compact. If $C_0(X)$ has an identity $e$ then $ef = f$ for every $f$, so $e(x) \neq 0$ at every $x$: were $e$ to vanish at a point, every $f$ of $C_0(X)$ would vanish there too, against the existence of a continuous compactly supported function nonzero at that point. With $e^2 = e$ this gives $e \equiv 1$, and since $e \in C_0(X)$ the constant function $1$ vanishes at infinity, which is exactly the compactness of $X$. In the compact case $C_0(X) = C(X)$, and in the non-compact locally compact case it is the standard non-unital commutative example of the operator-algebraic side; the identification of commutative $\mathrm{C}^*$-algebras with the algebras $C_0(X)$ is *Operator Algebras*, §*The Gelfand–Naimark Theorems*.

- **The zero algebra.** The algebra $\{0\}$ has $1 = 0$ and is the degenerate case of §*Trivial Cases and the Hypothesis $1 \neq 0$*; it is the unique unital algebra whose identity is also its zero element.

- **The commutator algebra $A^-$.** It has no identity for nonzero $A$: an identity $e$ of any algebra satisfies $e \cdot e = e$, while the product of $A^-$ is the commutator, so $e \cdot e = [e,e] = 0$ and hence $e = 0$, which forces $A = 0$. The construction is that of *Associative Algebras*, §*The Universal Enveloping Algebra*, and it shows that a non-associative algebra of the corpus may be non-unital as well, for a reason independent of the failure of associativity.

These are the non-unital algebras of the corpus, and they are not deficient for it: each is an algebra in the sense fixed in *Algebras*, and a construction that needs an identity is applied to its unitisation, §*The Unitisation*. A topological one among them may carry instead a net that plays the role of the identity in the limit, the subject of §*Approximate Identities*.

## Approximate Identities

A non-unital algebra has no identity, but one carrying a topology may carry a net that plays its role in the limit.

**Definition.** Let $A$ be a Banach algebra, in the sense of *Topological Algebras and Banach Algebras*, §*Banach Algebras*. A **left approximate identity** for $A$ is a net $(u_\alpha) \subseteq A$ with $\|u_\alpha f - f\| \to 0$ for every $f \in A$; it is **two-sided** if also $\|f u_\alpha - f\| \to 0$, and **bounded** with bound $M$ if $\|u_\alpha\| \leq M$ for all $\alpha$.

The definition is that of *The Convolution Algebra $L^1(G)$*, §*Approximate Identities and Amenability*, and it uses a norm and a limit, neither of which is available in Part I. That article proves that every $L^1(G)$ has a two-sided approximate identity with $u_\alpha \geq 0$, $\int_G u_\alpha = 1$ and $\|u_\alpha\|_1 = 1$, and that it can be chosen approximately invariant under left translation exactly when $G$ is amenable, Reiter's property $P_1$; an approximate identity of an ideal is used again in *Operator Algebras*, §*Ideals and Quotients*, where it is a net $(u_\lambda)$ with $0 \leq u_\lambda \leq 1$ in the ideal. The approximate identity is the topological substitute for the missing identity of the examples of §*The Non-Unital Algebras of the Corpus*: it recovers the statement $uf = f$ in the limit, and it is what replaces the identity in the representation theory of *The Convolution Algebra $L^1(G)$*.

**Remark.** The topological hypothesis is not incidental: the definition quantifies over a net and uses the norm, and no purely algebraic device of the same kind is available in a general non-unital algebra. For the algebras of Part I the unitisation of §*The Unitisation* supplies an identity, at the cost of enlarging the algebra, the new identity lying outside the original copy of $A$; the approximate identity supplies one only in the limit, and only when a topology is present.
...
The non-unital algebras of the corpus are the polynomials of positive degree, the square-zero ideals, the proper ideals that carry no identity of their own, the commutator algebra $A^-$, the convolution algebra $L^1(G)$ for non-discrete $G$, the compact operators $K(H)$ on an infinite-dimensional Hilbert space, $C_0(X)$ for locally compact non-compact $X$, and the zero algebra; each is pointed to its own article, and a proper ideal that does carry an identity, such as the augmentation ideal of $k[\mathbb{Z}/2]$ over a field of characteristic not $2$, is a unital algebra and not on the list. A topological algebra of the list carries instead of an identity an approximate identity, a notion defined by a norm limit and belonging to *The Convolution Algebra $L^1(G)$*; no purely algebraic analogue is defined in the corpus.
...
| $L^1(G)$, $\delta_e$ | The convolution algebra, unital exactly for discrete $G$ with identity $\delta_e$ |
| $K(H)$ | The compact operators, non-unital for infinite-dimensional $H$ and simple, *Operator Algebras* |
| $C_0(X)$, $C(X)$ | Functions vanishing at infinity, and the compact case, *Operator Algebras* |
...
| $(u_\alpha)$, $P_1$ | A (left, two-sided, bounded) approximate identity, and Reiter's property, *The Convolution Algebra $L^1(G)$* |
...
- Jacques Dixmier, *$\mathrm{C}^*$-Algebras* (North-Holland, 1977), for approximate identities in non-unital Banach and $\mathrm{C}^*$-algebras and the unitisation in the operator-algebraic setting.
