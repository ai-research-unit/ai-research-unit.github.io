...
# __Topological Modules and Vector Spaces__

## Introduction

A topological module is a module carrying a topology for which addition and scalar multiplication are continuous; the topology is determined by the neighbourhoods of zero, and when a fundamental system of these consists of submodules the topology is called linear. This is the general setting in which limits, Cauchy sequences and completions make sense for modules, and it contains the $p$-adic integers, the formal power series rings and the profinite groups as inverse limits of discrete quotients. A topological vector space is the same notion over a topological field, and the seminorms of functional analysis are the standard way of producing locally convex topologies on it.

Throughout, $R$ is a topological ring, commutative with $1 \neq 0$ unless stated, and $M$ is a topological $R$-module; the module topology is additive and translation-invariant. This article is the general topological theory. The normed theory, where the topology comes from a single norm and the Baire-based cornerstones are proved, is the companion article of this category on normed and Banach spaces, and the inner-product and Hilbert-space theory is the companion article on Banach and Hilbert spaces; a normed space is a special case of a topological vector space, not the general case.

## Topological Modules

### Definition

**Definition.** A **topological $R$-module** is an $R$-module $M$ equipped with a topology such that the addition $M \times M \to M$ and the scalar multiplication $R \times M \to M$ are continuous. Here $R$ is a topological ring and $R \times M$ and $M \times M$ carry the product topologies.
...
$$
\bigcap_{U \in \mathcal{B}} U=\{0\} ,
$$

and it is **discrete** if $\{0\}$ is a neighbourhood of $0$.

**Example.** Every module with the discrete topology is a topological module. The field $\mathbb{R}$ with its usual topology makes $\mathbb{R}^n$ a topological $\mathbb{R}$-module. A topological ring $R$ is a topological module over itself. Any submodule of a topological module with the subspace topology, and any quotient with the quotient topology, is again a topological module.

### Linear Topologies

**Definition.** A topology on $M$ is **linear** if $M$ has a fundamental system of neighbourhoods of $0$ consisting of submodules. A topology on $R$ or on $M$ given by a descending chain of submodules

$$
M=U_0 \supseteq U_1 \supseteq U_2 \supseteq \cdots
$$

is the topology in which a set is open if it is a union of cosets of the $U_n$; equivalently, the $U_n$ form a fundamental system of neighbourhoods of $0$.
...
**Definition.** A **Cauchy net** in $M$ is a net $(x_\lambda)$ such that for every neighbourhood $U$ of $0$ there is $\lambda_0$ with $x_\lambda-x_\mu \in U$ for $\lambda,\mu \ge \lambda_0$. $M$ is **complete** if every Cauchy net converges, and **separated** if Hausdorff.

**Theorem (completion).** Let $\mathcal{B}=\{U\}$ be a fundamental system of neighbourhoods of $0$ of a Hausdorff topological module $M$, consisting of submodules and directed downwards. Then the **completion** of $M$ is

$$
\widehat M=\varprojlim_{U \in \mathcal{B}} M/U ,
$$
...
**Example ($\mathbb{Z}_p$).** Let $p$ be a prime. The ring of **$p$-adic integers** is

$$
\mathbb{Z}_p=\varprojlim_n \mathbb{Z}/p^n\mathbb{Z} ,
$$

the inverse limit of the rings $\mathbb{Z}/p^n\mathbb{Z}$ with the reduction maps. It is the completion of $\mathbb{Z}$ for the $(p)$-adic topology, and the natural map $\mathbb{Z} \to \mathbb{Z}_p$ is injective with dense image.

**Proposition.** (i) Every element of $\mathbb{Z}_p$ is uniquely a series $\sum_{k \ge 0}a_kp^k$ with $a_k \in \{0,1,\dots,p-1\}$.

(ii) $\mathbb{Z}_p$ is an integral domain, its group of units is $\mathbb{Z}_p^{\times}=\mathbb{Z}_p \setminus p\mathbb{Z}_p$, and its maximal ideal is $p\mathbb{Z}_p$, with residue field $\mathbb{F}_p$; it is a local ring.

(iii) The field of fractions of $\mathbb{Z}_p$ is $\mathbb{Q}_p$, whose elements are the Laurent series $\sum_{k \ge -N}a_kp^k$.
...
**Definition.** A topological module is **profinite** if it is an inverse limit of finite discrete modules, equivalently if it is compact, Hausdorff, totally disconnected and complete. A topological module is **complete** if $\iota:M \to \widehat M$ is an isomorphism.

**Proposition.** (i) Inverse limits of profinite modules are profinite; products of profinite modules are profinite; a closed submodule and a Hausdorff quotient of a profinite module are profinite. (ii) $\mathbb{Z}_p$ is profinite, and the **profinite completion** of $\mathbb{Z}$,
...
**Definition.** Let $K$ be a topological field. A **topological vector space** over $K$ is a $K$-vector space $V$ with a topology such that addition $V \times V \to V$ and scalar multiplication $K \times V \to V$ are continuous. A subset $B \subseteq V$ is **bounded** if for every neighbourhood $U$ of $0$ there is $\lambda \in K^{\times}$ with $B \subseteq \lambda U$.

**Example.** $\mathbb{R}^n$ and $\mathbb{C}^n$ with the Euclidean topology are topological vector spaces; every normed space is a topological vector space, with neighbourhoods of $0$ the balls $\{x:\|x\|<\varepsilon\}$; the field $K$ with the discrete topology gives the discrete vector spaces, and for $K$ infinite the product $K^{\mathbb{N}}$ with the product topology is a non-discrete topological $K$-vector space, so a discrete field does not force its spaces to be discrete.

### Seminorms and Locally Convex Spaces

**Definition.** A **seminorm** on a $K$-vector space $V$, where $K$ is $\mathbb{R}$ or $\mathbb{C}$, is a function $p:V \to \mathbb{R}_{\ge0}$ with $p(x+y) \le p(x)+p(y)$ and $p(\lambda x)=|\lambda|p(x)$; it is a norm exactly when $p(x)=0$ implies $x=0$. A family $\mathcal{P}$ of seminorms gives the **locally convex topology** with a fundamental system of neighbourhoods of $0$ given by the finite intersections of the sets $\{x:p(x)<\varepsilon\}$, $p \in \mathcal{P}$, $\varepsilon>0$.
...
**Definition.** A **Fréchet space** is a complete Hausdorff locally convex topological vector space whose topology is metrizable, equivalently whose topology is given by a countable family of seminorms. A Banach space is a Fréchet space whose topology comes from a single norm.

**Example.** (i) $C^\infty(U)$ for an open $U \subseteq \mathbb{R}^n$, with the seminorms $p_{K,\alpha}(f)=\sup_{x \in K}|\partial^\alpha f(x)|$ over compact $K$ and multi-indices $\alpha$, is a Fréchet space; the topology of uniform convergence of all derivatives on compact sets. (ii) The Schwartz space $\mathcal{S}(\mathbb{R}^n)$ of rapidly decreasing smooth functions, with seminorms $\sup_x |x^\beta\partial^\alpha f(x)|$, is a Fréchet space. (iii) The space $C(K)$ of continuous functions on a compact space $K$ with the supremum norm is a Banach space, hence Fréchet. (iv) A countable product of Banach spaces is Fréchet and in general not Banach; an uncountable product of nontrivial Banach spaces is not metrizable.
...
A topological $R$-module is an $R$-module with a topology making addition and scalar multiplication continuous; the topology is translation-invariant and determined by the neighbourhoods of zero, and it is linear when a fundamental system of these consists of submodules. The linear topology associated to a descending chain of submodules, and in particular the $I$-adic topology with neighbourhoods $I^nM$, is the general framework; a linear topology is Hausdorff exactly when the intersection of the neighbourhoods is zero.

Quotients of topological modules carry the quotient topology and remain topological modules; the completion is the inverse limit $\widehat M=\varprojlim M/U$ over a fundamental system of neighbourhoods, and it is characterised by the universal property for uniformly continuous maps into complete Hausdorff modules, being complete and containing $M$ densely. The completion functor preserves finite products and is left exact on uniform embeddings of topological modules, and it is not exact in general; on finitely generated modules over a Noetherian ring with the $I$-adic topology it is exact, by the Artin–Rees lemma.

The $p$-adic integers $\mathbb{Z}_p=\varprojlim_n\mathbb{Z}/p^n\mathbb{Z}$ are the completion of $\mathbb{Z}$ in the $(p)$-adic topology and also in the metric of $|\cdot|_p$, with the strong triangle inequality; $\mathbb{Z}_p$ is a compact Hausdorff totally disconnected complete local domain with maximal ideal $p\mathbb{Z}_p$ and residue field $\mathbb{F}_p$, its field of fractions is $\mathbb{Q}_p$, and $\widehat{\mathbb{Z}}=\prod_p\mathbb{Z}_p$. Profinite modules are inverse limits of finite discrete modules and are exactly the compact Hausdorff totally disconnected complete modules, and over a Noetherian ring the $I$-adic completion of a finitely generated module is complete, Hausdorff and separated.

A topological vector space over a topological field is the vector-space case of the same definition; over $\mathbb{R}$ or $\mathbb{C}$ a family of seminorms produces a locally convex topology, which is Hausdorff when the seminorms separate points, and a Fréchet space is a complete metrizable locally convex space, equivalently one defined by countably many seminorms. Normed spaces are the special case of a family consisting of a single norm, and their theory, with the equivalence of norms in finite dimension and the Baire-based cornerstones, is the companion article on normed and Banach spaces.

## Summary of Notation

| Symbol | Meaning |
|---|---|
| $R$ | a topological ring |
| $M$ | a topological $R$-module |
| $\mathcal{B}$, $U$ | fundamental system of neighbourhoods of $0$ |
| $I^nM$ | $I$-adic neighbourhoods of zero |
| $\widehat M = \varprojlim M/U$ | completion |
| $\mathbb{Z}_p$, $\mathbb{Q}_p$ | $p$-adic integers and numbers |
| $\widehat{\mathbb{Z}} = \prod_p\mathbb{Z}_p$ | profinite completion of $\mathbb{Z}$ |
| $|\cdot|_p$, $v_p$ | $p$-adic absolute value and valuation |
| $K$ | a topological field |
| $p$, $\mathcal{P}$ | seminorm and family of seminorms |
| $C^\infty(U)$, $\mathcal{S}(\mathbb{R}^n)$ | Fréchet spaces of functions |

## Further Reading

- Nicolas Bourbaki, *General Topology: Chapters 1–4* (Springer, 1995), for uniform spaces and completions.
- Nicolas Bourbaki, *Commutative Algebra: Chapters 1–7* (Springer, 1989), for the $I$-adic topology and completions.
- John B. Conway, *A Course in Functional Analysis* (Springer, 2nd ed. 1990), for Fréchet spaces and locally convex theory.
- Jean Dieudonné, *Treatise on Analysis, Vol. II* (Academic Press, 1970), for topological vector spaces.
- Israel M. Gelfand, Dmitri A. Raikov and Georgi E. Shilov, *Commutative Normed Rings* (Chelsea, 1964), for topological rings and their completions.
- Alexander Grothendieck, *Topological Vector Spaces* (Gordon and Breach, 1973), for general locally convex theory.
- Jürgen Neukirch, *Algebraic Number Theory* (Springer, 1999), for $\mathbb{Z}_p$, $\mathbb{Q}_p$ and profinite completions.
- Jean-Pierre Serre, *Local Fields* (Springer, 1979), for the $p$-adic absolute value and its completions.
