...
This article determines the roots of $-1$ in the split biquaternion algebra $\mathbb{H}_{\mathbb{D}}$, that is, the elements $\xi \in \mathbb{H}_{\mathbb{D}}$ satisfying
...
The reason for the difference is that the split complex algebra $\mathbb{D}$ has no square root of $-1$, so there is no "scalar imaginary" root. In the biquaternion case, the scalar imaginary $i$ is a root of $-1$, and it generates the non-trivial roots by combination with the quaternion roots. In the split biquaternion case, there is no such scalar root, and the root set is simply the product of two quaternion root sets.
...
This is a reflection of the fact that the biquaternion algebra is simple (and its idempotents come from the roots of $-1$), while the split biquaternion algebra is semisimple (and its idempotents come from the split complex algebra).
