...
$$
\mathcal{H} = \{u \in \mathbb{D} : N(u) = 1\} = \{u = x + jy : x^2 - y^2 = 1\}.
$$
...
$$
x' = x\cosh\phi + y\sinh\phi, \qquad y' = x\sinh\phi + y\cosh\phi.
$$
...
and the following table describes them, where $\operatorname{Re} u$ denotes the first column entry $x$ of the corresponding $u = x + jy$.
