...
which is a homeomorphism invariant and which agrees for $L(p,q)$ and $L(p,q')$ exactly when $q' \equiv \pm q^{\pm1}\pmod p$. The sufficiency is by exhibiting the explicit homeomorphisms: the changes $q\mapsto -q$ and $q\mapsto q^{-1}$ are realised by the reflections of the sphere. $\square$
...
**Proof.** The fundamental group is computed from the free action; the homology from the cell structure of the quotient or from the Poincaré duality $H_2\cong H^1 = \mathrm{Hom}(H_1,\mathbb{Z}) = 0$; the linking form is computed from the intersection form of the plumbing of (d) above, restricted to the torsion of the boundary. $\square$
...
**Proof sketch.** The spherical structure is the quotient structure; the spectral computation is the Peter–Weyl decomposition of $L^2(S^3)$ into the representations of $S^3$, restricted to the cyclic subgroup. The analysis of the spectrum and the heat kernel is part of the spectral theory of Part III. $\square$
