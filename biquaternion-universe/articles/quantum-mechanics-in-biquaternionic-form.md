...
The companion article, *Quantum Mechanics: Foundations and Structure*, presented standard quantum mechanics in its conventional form: states are density matrices on a complex Hilbert space, observables are Hermitian operators, dynamics is unitary, and measurement is projective. It closed with a checklist of ten structural requirements that any comprehensive quantum theory must satisfy.

This article presents quantum mechanics in the biquaternion framework. The central object is the **Hermitian subspace** $\mathbb{M}_+$ of the biquaternion algebra $\mathbb{B} = \mathbb{C}\otimes_\mathbb{R}\mathbb{H}$. Its elements are both the **states** and the **observables** of the theory. This is a structural feature that standard quantum mechanics does not have: in the standard formalism, states and observables are different kinds of objects (functionals on one side, operators on the other), while in the biquaternion framework they are elements of the same four-dimensional real subspace.
...
**Trace.** The **trace** of an element of $\mathbb{M}_+$ is twice its scalar part:
...
The result is a pure real quaternion (vector part only, with real coefficients), lying in the intersection $\mathbb{M}_- \cap \mathbb{H}_{\mathbb{B}}$, which is the real span of $e_1, e_2, e_3$. This is the biquaternion form of the canonical commutation relations of the qubit.
...
where $\tilde{U}(t)$ is a unit-norm biquaternion, $\tilde{U}\tilde{U}^\dagger = e_0$, satisfying the **Schrödinger equation**
...
The element $\tilde{U}(t)$ is a **unit-norm biquaternion**: $\tilde{U}(t)\tilde{U}(t)^\dagger = e_0$. It is not Hermitian (unless $\mathbf{h} = 0$); it is a general element of the unitary group $U(2) \subset \mathbb{B}$.
...
**Interpretation.** The unitary $\tilde{U}(t)$ is an inner automorphism of the algebra: conjugation by a unit-norm biquaternion preserves multiplication. Its action on $\mathbb{M}_+$ keeps the scalar part fixed and rotates the imaginary vector part, giving the standard precession of a spin-1/2 in a magnetic field, expressed as the biquaternion rotation.
...
The evolution generated by a Hermitian Hamiltonian is **unitary**: the element $\tilde{U}(t)$ has unit norm. The evolution is therefore **reversible**: the state at any later time can be recovered from an earlier time by applying $\tilde{U}(t)^\dagger$.
...
This is a Lie algebra structure on $\mathbb{M}_-$ (which is not a subalgebra of $\mathbb{B}$, but is a Lie algebra under the commutator). The exponential map sends $\mathbb{M}_-$ to the group of unit-norm biquaternions.
...
**6. Composition rule.** The tensor product of two qubits corresponds to $\mathbb{B}\otimes\mathbb{B} \cong M_4(\mathbb{C})$, and the partial trace is defined in terms of the standard trace on the tensor product. This is a natural extension of the framework.
...
**8. Relativistic extension.** The biquaternion algebra contains the Lorentz group $SL(2,\mathbb{C})$ as the group of unit-norm biquaternions, and the material sector $\mathbb{M}_-$ carries the four-vectors. The algebraic structure required for the Lorentz group and the spinor representation is present in the algebra. The quantum formalism presented here and the relativistic structure share the same algebraic home, which makes the framework naturally suited to a relativistic quantum theory of spinor fields. The classical precursor of such a theory — the biquaternion Dirac equation — is treated in the companion article, and the quantization of the spinor field is a natural direction for future work.
...
7. **The framework contains the algebraic structure for relativity.** The Lorentz group is contained in the algebra as the group of unit-norm biquaternions, and the material sector $\mathbb{M}_-$ carries the four-vectors. The biquaternion framework is therefore the natural setting in which the quantum formalism and the relativistic structure coexist, without the incompatibilities that appear when they are formulated separately.

## Open Questions

The idempotent formalism raises several questions.

**1. Extension to many qubits.** The formalism above describes a single qubit. The extension to $n$ qubits requires the tensor product $\mathbb{B}^{\otimes n}$, which is isomorphic to $M_{2^n}(\mathbb{C})$. How do the idempotents, states, and observables generalize? Is the tensor product natural in the biquaternion framework, or does it require additional structure?
...
The biquaternion algebra contains the algebraic structure required for relativity — the Lorentz group, the spinor representation, and the material sector of four-vectors — and the quantum formalism presented here and the relativistic structure coexist within the same algebra. The full relativistic quantum theory of spinor fields is the natural continuation of the framework: the classical precursor is the biquaternion Dirac equation treated in the companion article, and the quantization of the spinor field is a natural direction for future work.
