...
This article introduces harmonic analysis for split-quaternion-valued functions of a real variable. It follows the article on split quaternion discrete harmonic analysis, which defined the discrete split-quaternion Fourier transform, and it uses the analysis article, which defined the split-quaternion gradient, the d'Alembertian, and the convective derivative on the four-dimensional subspaces of $\mathbb{H}_{\mathbb{D}}$.

The treatment is purely mathematical. The goal is to define the continuous split-quaternion Fourier transform, establish its basic properties, show how it decomposes into ordinary complex Fourier transforms via the idempotent decomposition, and identify the points where the split quaternion structure creates genuinely new phenomena. The relation to the differential operators of the analysis article is the main structural content of this article.

The key structural fact is the same as in the discrete case: the **idempotent decomposition** of the split quaternion algebra reduces the transform to two independent quaternion Fourier transforms, one for each idempotent component. The split complex unit $j$ does not appear in the kernel; it only appears in the idempotent decomposition that separates the two components. This is the fundamental simplification relative to the biquaternion case.
...
**Notation.** To avoid collision with the standard basis $\{e_0, e_1, e_2, e_3\}$ and with the split complex unit $j$, the root of $-1$ used in the Fourier kernel is denoted $\rho$ throughout. This is a local convention; the roots themselves are the objects classified in the article on split quaternion roots of minus one.
...
The **split quaternion Fourier kernel** is the function
...
The kernel is the same in both idempotent components, so the split quaternion kernel is the diagonal embedding of the quaternion kernel. This is the same as in the discrete case.
...
Let $f : \mathbb{R} \to \mathbb{H}_{\mathbb{D}}$ be a split-quaternion-valued function. The **continuous split-quaternion Fourier transform** of $f$ is
...
**Theorem (Plancherel).** The transform extends uniquely to a unitary operator on the space $L^2(\mathbb{R}, \mathbb{H}_{\mathbb{D}})$ of square-integrable split-quaternion-valued functions, with
...
This is the fundamental simplification: the split quaternion Fourier transform reduces to **two independent quaternion Fourier transforms**, one for each idempotent component.
...
This is the total number of complex transforms required for a general split-quaternion-valued function.
...
- **General split quaternion:** The function has eight real components. The transform requires **eight complex Fourier transforms**.

### Why This Matters

The factorization shows that the split quaternion Fourier transform is not a fundamentally new operation: it is the quaternion Fourier transform applied to the two idempotent components, dressed in split quaternion language. The non-trivial content is the **choice of root** $\rho$ and the **idempotent decomposition**. Everything else is ordinary harmonic analysis.

## The Convolution Theorem

### Definition of Convolution

Let $f, g : \mathbb{R} \to \mathbb{H}_{\mathbb{D}}$ be two split-quaternion-valued functions. The **convolution** of $f$ and $g$ is

$$
(f * g)(t) = \int_{-\infty}^{\infty} f(\tau) g(t - \tau) \, d\tau,
$$

provided the integral converges. The convolution is **not commutative** in general, because the split quaternion product is not commutative.

### The Convolution Theorem

**Theorem (convolution theorem).** Let $f, g : \mathbb{R} \to \mathbb{H}_{\mathbb{D}}$ be integrable, and let $\mathcal{F}$ denote the continuous split-quaternion Fourier transform with the kernel on the left:
...
The convolution theorem implies that convolution in the time domain becomes **split quaternion multiplication** in the frequency domain. The convolution is associative, distributive over addition, and not commutative.
...
where $*$ on the right is the quaternion convolution. So the split quaternion convolution is the pair of the quaternion convolutions of the two idempotent components. This is the cleanest form of the convolution theorem.
...
Let $\tilde{\nabla}$ be the split quaternion gradient on a four-dimensional subspace $V \subset \mathbb{H}_{\mathbb{D}}$, with coordinates $x_0, x_1, x_2, x_3$. For a function $\tilde{F} : V \to \mathbb{H}_{\mathbb{D}}$, the Fourier transform in the variable $x_0$ (with the other coordinates treated as parameters, or with a full four-dimensional transform) diagonalizes the partial derivative $\partial_0$:
...
If the transform is taken in all four variables, the gradient becomes multiplication by the split quaternion

$$
2\pi \rho (\omega_0 e_0 + \omega_1 e_1 + \omega_2 e_2 + \omega_3 e_3),
$$

where $\omega_0, \omega_1, \omega_2, \omega_3$ are the frequency variables conjugate to $x_0, x_1, x_2, x_3$. The precise form depends on the placement of the root $\rho$ and on the choice of basis, but the structural fact is that the gradient becomes a split-quaternion-valued multiplier.
...
The convective derivative $\tilde{D} = \bar{\tilde{U}} \tilde{\nabla}$ becomes multiplication by the split quaternion

$$
\bar{\tilde{U}} \cdot 2\pi \rho (\omega_0 e_0 + \omega_1 e_1 + \omega_2 e_2 + \omega_3 e_3),
$$

which depends on the velocity split quaternion $\tilde{U}$ and on the frequency variables.

### The General Principle

The general principle is that the continuous Fourier transform **diagonalizes the constant-coefficient differential operators** on the four-dimensional subspaces. The symbol of the operator is a split-quaternion-valued function of the frequency variables, and the solutions of the operator equation are determined by the support of the transform on the zero set of the symbol.
...
where $\tilde{\nabla}$ on the right is the quaternion gradient acting on each component. The d'Alembertian and the convective derivative act in the same way. So the split quaternion analysis is the quaternion analysis applied to each of the two idempotent components separately, and the Fourier transform diagonalizes the operators in each component.
...
This is the standard Poisson summation formula, and it holds in the split quaternion case with the same proof, because the kernel is the ordinary complex kernel in the direction $\rho$.

### The Sampling Theorem

The **sampling theorem** (or Shannon–Nyquist theorem) states that a function whose continuous transform is supported in a bounded interval $[-W, W]$ can be reconstructed from its samples at the rate $1/(2W)$. The split quaternion version of the sampling theorem holds under the same conditions, with the reconstruction formula
...
4. **Wavelets and time-frequency analysis.** Can a split quaternion wavelet transform be defined, and what are its properties? Can a split quaternion time-frequency distribution be defined?
...
6. **The Clifford algebra framework.** How does the split quaternion continuous Fourier transform fit into the general theory of Clifford algebra Fourier transforms with split signature?
...
The continuous split-quaternion Fourier transform is defined by the kernel $W(t, \omega) = \exp(-2\pi \rho \omega t)$, where $\rho$ is a unit pure real quaternion (a root of $-1$ in the quaternion algebra). The kernel is the same in both idempotent components, so the transform decomposes into two independent quaternion Fourier transforms, one for each component.

The transform is invertible on functions where both idempotent components are nonzero almost everywhere, and it satisfies the Riemann–Lebesgue lemma, the inversion theorem, and the Plancherel theorem. It factorizes into eight complex Fourier transforms for a general split-quaternion-valued function, four for a quaternion-valued function, and two for a split-complex-valued function. The convolution theorem holds, with the non-commutativity inherited from the quaternion case.

The transform diagonalizes the constant-coefficient differential operators of the analysis article: the gradient becomes a split-quaternion-valued multiplier, the d'Alembertian becomes a scalar multiplier vanishing on the light cone, and the convective derivative becomes a split-quaternion-valued multiplier depending on the velocity. In the idempotent basis, the operators act componentwise, and the transform diagonalizes them in each quaternion component.

The vanishing-norm issue is a genuinely split quaternion feature: functions that take zero-divisor values on a set of positive measure are not necessarily recoverable from their transform, and the precise invertibility condition is not known.
