...
The earlier exercises treated a single qubit (Exercises: Measuring Spin Along an Arbitrary Direction, Spin Precession in a Magnetic Field and Successive Measurements of Spin), an entangled pair of qubits in the singlet state (Exercise: Two Spins in the Singlet State), and the reduced state of one subsystem (Exercise: The Reduced State of an Entangled Subsystem). This exercise treats **all four Bell states** — the four maximally entangled two-qubit states — and the **spin correlation function**, the quantity that encodes the full dependence of joint measurement outcomes on the two measurement directions.
...
They form an orthonormal basis of the two-qubit Hilbert space $\mathbb{C}^2\otimes\mathbb{C}^2$. The singlet $|\Psi^-\rangle$ was treated in Exercise: Two Spins in the Singlet State; the other three are its companions.
...
So $T_{\Psi^-} = -\mathrm{diag}(1,1,1)$, and $E = -\hat{a}\cdot\hat{b}$. This reproduces the result of Exercise: Two Spins in the Singlet State.
...
This follows from $\langle \sigma_j \otimes I\rangle = \langle I \otimes \sigma_k\rangle = 0$ in every Bell state: the single-particle expectations vanish, and the Born probabilities reduce to $\tfrac{1}{4}(1 \pm E)$. Exercise: Two Spins in the Singlet State's probability $p(+,+) = \tfrac{1}{4}(1 - \hat{a}\cdot\hat{b})$ is the special case $E = -\hat{a}\cdot\hat{b}$ for the singlet.

**Consistency with Exercise: Two Spins in the Singlet State.** For the singlet, $E(\hat{a}, \hat{b}) = -\hat{a}\cdot\hat{b}$, so $p(+,+) = \tfrac{1}{4}(1 - \hat{a}\cdot\hat{b})$, recovering Exercise: Two Spins in the Singlet State.
...
**2. The correlation function is a bilinear pairing.** The correlation function $E(\hat{a}, \hat{b}) = \mathrm{Tr}(P_\epsilon\circ\tilde{H}_{ab})$ is the tensor-product trace pairing between the state idempotent and the observable $\tilde{H}_{ab} = (i\hat{a})\otimes(i\hat{b})$. The trace pairing is the same algebraic operation that gives the Born rule (Exercise: Measuring Spin Along an Arbitrary Direction) and the expectation value (Exercise: Spin Precession in a Magnetic Field).
...
## Comparison with Exercises: Measuring Spin Along an Arbitrary Direction, Spin Precession in a Magnetic Field, Reduced State of an Entangled Subsystem, Successive Measurements of Spin and Two Spins in the Singlet State

| | Measuring Spin Along an Arbitrary Direction | Spin Precession in a Magnetic Field | Successive Measurements of Spin | Two Spins in the Singlet State | Reduced State of an Entangled Subsystem | Correlation Function of the Bell States |
|---|---|---|---|---|---|---|
