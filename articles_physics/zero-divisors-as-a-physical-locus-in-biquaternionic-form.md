...
\tilde Q\ne0,
\qquad
N(\tilde Q)=\tilde Q\bar{\tilde Q}=\sum_\mu Q_\mu^2=0 ,
$$

and the nonzero elements split into two disjoint families:

$$
\text{pure }(Q_0=0):\quad Q_1^2+Q_2^2+Q_3^2=0,\quad \tilde Q^2=0 ;
\qquad
...
$$
\tilde P=\tfrac12(e_0+i\hat{\boldsymbol\mu})\ \ \text{for a unit vector}\ \hat{\boldsymbol\mu},
\qquad
\tilde P^2=\tilde P,
\qquad
N(\tilde P)=\tilde P\bar{\tilde P}=0 ,
$$

so $\tilde P$ is a non-pure zero divisor and a **right** zero divisor with annihilator $\bar{\tilde P}$, and $\bar{\tilde P}$ is a left zero divisor with annihilator $\tilde P$. These identities were verified exactly for $\hat{\boldsymbol\mu}=\hat{\mathbf e}_1$: $\tilde P^2=\tilde P$, $N(\tilde P)=0$, and both products $\tilde P\bar{\tilde P}$ and $\bar{\tilde P}\tilde P$ vanish in explicit quaternion arithmetic. The physical state space is the left module $\mathbb{B}\tilde P$, and its generator is a null element of the algebra. The zero divisor locus is therefore not only the boundary of the state space; it is its origin. The idempotents that project onto the two-component module, and hence the module itself, are supplied by the cone.
...
$$
\tilde{\mathcal P}=\frac{E}{ic}\,e_0+\mathbf p ,
\qquad
...
$$
x_\pm=\pm\sqrt{\frac{2E}{m\omega^2}} ,
\qquad
p(x_\pm)=\sqrt{2m(E-V(x_\pm))}=0 .
$$
...
For a field taking values in the state module this vanishing is automatic rather than special: the module is generated by the idempotent $\tilde P$ of the previous section, $N(\tilde P)=0$, so every module element $\tilde Q=\tilde A\tilde P$ has $N(\tilde Q)=\tilde A\tilde P\bar{\tilde P}\bar{\tilde A}=0$. The construction below isolates the algebraic cause by allowing $\tilde Q$ to range over the whole algebra.
...
The zero divisor set of $\mathbb{B}$ is $\mathcal{Z}=\{\tilde Q\ne0:N(\tilde Q)=0\}$, a complex cone of complex dimension $3$ minus the origin, split into the pure nilpotents ($Q_0=0$, $\tilde Q^2=0$) and the non-pure multiples of idempotents ($Q_0\ne0$, $\tilde Q^2=2Q_0\tilde Q$, $\tilde P=\tilde Q/2Q_0$); both identities were verified exactly for $\tilde Q=e_1+ie_2$ and $\tilde Q=e_0+ie_1$. The cone lies in $\mathbb{M}_\pm$ and is absent from $\mathbb{C}_{\mathbb{B}}$ and $\mathbb{H}_{\mathbb{B}}$; in $\mathbb{M}_-$ with $\tilde X=ict\,e_0+\mathbf x$ it is the light cone.
...
| $\tilde X=ict\,e_0+\mathbf x$ | Material coordinate; $\mathbb{M}_-$ cone is the light cone |
...
| $\flat=-\dagger$ | The algebra's real structure |
