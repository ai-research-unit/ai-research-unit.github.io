# __Goldstone's Theorem in Biquaternionic Form__
...
$$
\hat U_\alpha=\exp\!\left(i\alpha\hat Q\right),
\qquad
...
$$
\langle0|\,\hat{\tilde{\Phi}}\,|0\rangle=v\,e^{i\alpha_0}\,e_0
\equiv \tilde v\,e_0
$$
...
$$
\langle\hat{\phi}_1\rangle=\sqrt2\,v ,
\qquad
\langle\hat{\phi}_2\rangle=0 ,
\qquad
\big|\langle\hat{\tilde{\Phi}}\rangle\big|=v\,e_0 .
$$
...
$$
V=\frac{\lambda}{4}\left(\frac{(\sqrt2 v+h)^2+\theta^2}{2}-v^2\right)^2
=\frac{\lambda}{4}\left(\sqrt2\,v\,h+\frac{h^2}{2}+\frac{\theta^2}{2}\right)^2 .
$$

Expanding to second order in the fluctuations,

$$
V=\frac{\lambda}{2}\,v^2 h^2+O(h^3,h\theta^2,\dots)
...
$$
J^\mu=-\left(\phi_1\partial^\mu\phi_2-\phi_2\partial^\mu\phi_1\right)
\;\xrightarrow{\ \text{expansion}\ } \;
J^\mu=-\sqrt2\,v\,\partial^\mu\theta+\theta\,\partial^\mu h-h\,\partial^\mu\theta ,
$$
...
$$
J^\mu=-f\,\partial^\mu\theta+O(\text{two fields}),
\qquad
f=\sqrt2\,v ,
$$
...
**Verification.** The current identity is exact, and its remainder is explicit rather than numerical: with $\phi_1=\sqrt2v+h$, $\phi_2=\theta$ and no approximation,
$$
-(\phi_1\dot\phi_2-\phi_2\dot\phi_1)=-f\dot\theta-\big(h\,\dot\theta-\theta\,\dot h\big),
\qquad
f=\sqrt2\,v,
$$
...
$$
\mathcal{L}_{\mathrm{eff}}=\frac{1}{2}\,\partial_\mu\theta\,\partial^\mu\theta
+\frac{c_4}{f^4}\,\big(\partial_\mu\theta\,\partial^\mu\theta\big)^2+\cdots ,
\qquad
f=\sqrt2\,v ,
$$
...
$$
\big[\hat Q,\hat h\big]=-i\hat\theta ,
\qquad
\langle0|\big[\hat Q,\hat\theta(0)\big]|0\rangle=i f ,
\qquad
f=\sqrt2\,v ,
$$
...
**The flatness is the flatness of the norm form on the phase orbit.** The potential is a function of the central invariant $\mathrm{Sc}(\tilde{\Phi}^\dagger\tilde{\Phi})$, and the vacuum manifold is its level set, a circle. The radial and angular directions of the central field space are, respectively, the massive and massless directions, and the masslessness is the exact flatness of the potential along the orbit.

**The algebra supplies no new mechanism.** The theorem, its proof, the decay constant and the counting are standard, and the framework contributes the centrality of the symmetry, the order parameter and the Goldstone mode, plus the norm-form geometry of the invariant. The scalar sector's usual structural result persists: no native ladder in $\mathbb{B}$, and the Goldstone boson is a standard scalar constructed on the imported formalism.
...
- **The decay constant's normalization.** The decay constant is the order parameter in units fixed by the kinetic term; whether the framework's norm form or trace form fixes a preferred normalization, and hence a preferred value of $f$ relative to $v$, is not shown.
